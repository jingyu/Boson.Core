...
public class ActiveProxyTest {
	...
	private static final String regularNodeExecutable = "/Users/jingyu/Projects/Boson/Native/build/apps/launcher/boson-launcher";
	...
	private static void setupSuperNode() throws Exception {
		...
		io.bosonnetwork.Configuration config = new DefaultConfiguration.Builder()
	...
	private static File createRegularNodeConfig() throws IOException {
		...
		config.put("logger", JsonObject.of(
				"level", "debug",
				"logFile", dir.getAbsolutePath() + File.separator + "boson.log",
	...
	private static void setupRegularNode() throws Exception {
		...
		regularNodeProcess.onExit().thenApply(p -> {
			System.out.println("Regular node[boson-launcher] exited, exit code = " + p.exitValue());
			return null;
		});
	}
